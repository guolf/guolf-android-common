...
public class OkHttpClientManager {
    ...
    private void deliveryResult(ResultCallback callback, Request request) {
        ...
        mOkHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(final Request request, final IOException e) {
                sendFailedStringCallback(request, e.getMessage(), resCallBack);
            }
    ...
    private void sendFailedStringCallback(final Request request, final String e, final ResultCallback callback) {
    ...
    private void sendProgressResultCallback(final int progress, final ResultCallback callback) {
        mDelivery.post(new Runnable() {
            @Override
            public void run() {
    ...
    public static abstract class ResultCallback<T> {
        ...
        public abstract void onError(Request request, String error);
    ...
    public class DownloadDelegate {
        ...
        public void downloadAsyn(final String url, final String destFileDir, final ResultCallback callback, Object tag) {
            ...
            call.enqueue(new Callback() {
                @Override
                public void onFailure(final Request request, final IOException e) {
                    sendFailedStringCallback(request, "下载文件出错:" + e.getMessage(), callback);
                }
                ...
                public void onResponse(Response response) {
                    if (response.code() == 200) {
                        ...
                        LogUtils.i("下载异常 status code:" + response.code());
                        sendFailedStringCallback(response.request(), "下载文件出错，状态码:" + response.code(), callback);
                    }
                }
            });
        }
